...
class TxtDatasetProcessing(Dataset):
    def __init__(self, data_path, txt_path, txt_filename, label_filename, sen_len, corpus, target_class , reorganize = False):
        ...
        fp_label.close()
        if reorganize:
            self.label = [1 if label == target_class else 0 for label in labels]
        else:
            self.label = labels
        self.corpus = corpus
