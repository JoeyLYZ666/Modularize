...
def load_corpus(dataset_str):
    ...
    objects = []
    for i in range(len(names)):
        with open("D:\\Lab\\BUAA\\Demo\\other\\GNN-for-text-classification-master\\data/ind.{}.{}".format(dataset_str, names[i]), 'rb') as f:
            if sys.version_info > (3, 0):
                objects.append(pkl.load(f, encoding='latin1'))
            else:
                objects.append(pkl.load(f))
    ...
    train_idx_orig = parse_index_file(
        "D:\\Lab\\BUAA\\Demo\\grad-application\\data/{}.train.index".format(dataset_str))
...
def construct_graph(adjacency):
        ...
        weight = torch.from_numpy(adj.data.astype(np.float32)) # 邻接矩阵（0，1）
